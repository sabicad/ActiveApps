...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == self.currentActiveIndex) {
        attributes.size = CGSizeMake(90, 105);
        attributes.center = CGPointMake(_center.x, _center.y);
    } else {

        NSInteger index = indexPath.item;
        
        if (index > self.currentActiveIndex - 1) {
            index -= 1;
        }
        
        attributes.size = CGSizeMake(50, 65);
        attributes.center = CGPointMake(_center.x + _radius *
                                        cosf(2 * index * M_PI / (_cellCount - 1)),
                                        _center.y + _radius *
                                        sinf(2 * index * M_PI / (_cellCount - 1)));
    }
    return attributes;
}
...
- (UICollectionViewLayoutAttributes *)initialLayoutAttributesForAppearingItemAtIndexPath:(NSIndexPath *)itemIndexPath
{
    ...
    if ([insertPaths containsObject:itemIndexPath]) {
        ...
        attributes.center = CGPointMake(_center.x, _center.y);
        attributes.size = CGSizeMake(50 * 2, 65 * 2);
